...
import pandas as pd
import os
from models import (Production, Prestation,Resume)
import warnings
...
def resume(file_benef, file_emission, file_conso, seuil_ctx, ante=pd.to_datetime(str(start_date_entry.get())), post=pd.to_datetime(str(end_date_entry.get()))):
    ...
    Resume().ecart(selected_beneficiaire_path, selected_emission_path,selected_consommation_path,COUTIER_entry.get(),end_date_entry.get(),seuil_entry.get())
    return
