...
class Resume:
    ...
    def ecart(self, file_benef, file_cotisation, file_conso, courtier,date_fin,conso_ctx):
        ...
        #PRESTATIONS HORS PERIODE COUVERTURE
        ...
        benef_sans_date_naiss = benef_sans_doublon[benef_sans_doublon['dateNaissance'].isna()]["numPolice"].apply(
            lambda x: str(x)).tolist()
        ...
        conso_sorted = consommation_impt.sort_values(by='montantPaye', ascending=False).drop_duplicates(
            subset=['montantPaye', 'numPolice'], keep='first')
        presta_importantes = set(conso_sorted['numPolice'].tolist())

        #PRESTA AVEC DATE SOINS HORS PERIODE: COTISATION x CONSO
        ...
        presta_x_benef = pd.merge(consommation, benef, on='numPolice', how='inner')
        ...
        with pd.ExcelWriter(dossier_complet+'/'+'ECARTS.xlsx', engine='xlsxwriter') as writer:
            try:
                if len(presta_sans_cotisation) != 0:
                    consommation[consommation["numPolice"].isin(presta_sans_cotisation)].to_excel(writer, sheet_name="PRESTA_SANS_COTISATION.xlsx", index=False)
            except:
                #print('PRESTA_SANS_COTISATION vide')
                pass
            try:
                if len(presta_hors_periode_couv) != 0:
                    consommation[consommation['numPolice'].isin(presta_hors_periode_couv)].to_excel(writer, sheet_name="PRESTA_HORS_PERIODE_CVT.xlsx", index=False)
            except:
                #print('PRESTA_HORS_PERIODE_CVT vide')
                pass
            try:
                if len(ENFANT_SUP_25_CONSO) != 0:
                    consommation[consommation['numPolice'].isin(ENFANT_SUP_25_CONSO)].to_excel(writer, sheet_name="PRESTA_ENFANTS_SUP_25_ANS.xlsx", index=False)
            except:
                #print('PRESTA_ENFANTS_SUP_25_ANS vide')
                pass
            try:
                if len(ADULTES_SUP_60_CONSO) != 0:
                    consommation[consommation['numPolice'].isin(ADULTES_SUP_60_CONSO)].to_excel(writer, sheet_name="PRESTA_ADULTES_SUP_60_ANS.xlsx", index=False)
            except:
                #print('PRESTA_ADULTES_SUP_60_ANS vide')
                pass
            try:
                if len(benef_sans_date_naiss) != 0:
                    consommation[consommation['numPolice'].isin(benef_sans_date_naiss)].to_excel(writer, sheet_name="PRESTA_SANS_DATE_NAISS.xlsx", index=False)
            except:
                #print('PRESTA_SANS_DATE_NAISS vide')
                pass
            try:
                if len(dble_conso) != 0:
                    consommation[consommation['numPolice'].isin(dble_conso)].to_excel(writer, sheet_name="DOUBLE_CONSOMMATION.xlsx", index=False)
            except:
                #print('DOUBLE_CONSOMMATION vide')
                pass
            try:
                if len(liste_sans_date_emission) != 0:
                    consommation[consommation['numPolice'].isin(liste_sans_date_emission)].to_excel(writer, sheet_name="SANS_DATE_EMISSION.xlsx", index=False)
            except:
                #print('SANS_DATE_EMISSION vide')
                pass
            try:
                if len(liste_hors_delai_contractuel) !=0:
                    consommation[consommation['numPolice'].isin(liste_hors_delai_contractuel)].to_excel(writer, sheet_name="PRESTA_HORS_DELAI_CONTRACTUEL.xlsx", index=False)
            except:
                #print('PRESTA_HORS_DELAI_CONTRACTUEL vide')
                pass
            try:
                if len(presta_importantes) != 0:
                    consommation[consommation['numPolice'].isin(presta_importantes)].to_excel(writer, sheet_name="PRESTA_COUTEUSES.xlsx", index=False)
            except:
                #print('PRESTA_COUTEUSES vide')
                pass
            try:
                if len(liste_presta_hors_periode) != 0:
                    consommation[consommation["numPolice"].apply(lambda x: str(x).replace(" ","")).isin(liste_presta_hors_periode)].to_excel(writer, sheet_name="PRESTA_HORS_PERIODE.xlsx", index=False)
            except:
                #print('PRESTA_HORS_PERIODE vide')
                pass
            try:
                if len(liste_presta_dateSoins_apres_dateSortie) != 0:
                    consommation[consommation["numPolice"].apply(lambda x: str(x).replace(" ","")).isin(liste_presta_dateSoins_apres_dateSortie)].to_excel(writer, sheet_name="PRESTA_APRES_DATE_SOINS.xlsx", index=False)
            except:
